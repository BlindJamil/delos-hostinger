        ...
        <div class="bg-white rounded-xl border border-delos-dark/5 shadow-card p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="type" list="project-type-options" value="{{ old('type', $project->type) }}" placeholder="kitchens, bedroom, turnkey, ..."
                           class="w-full px-3 py-2 border border-delos-dark/15 rounded-lg text-sm focus:outline-none focus:border-delos-gold focus:ring-2 focus:ring-delos-gold/15 transition-all">
                    <datalist id="project-type-options">
                        @foreach(($projectTypeOptions ?? ['kitchens', 'living room', 'bedroom', 'wardrobes', 'dining room', 'turnkey']) as $opt)
                            <option value="{{ $opt }}">
                        @endforeach
                    </datalist>
                    <p class="text-xs text-delos-muted mt-1">Used by the public filter. Saved as lowercase-trimmed. Keep consistent across projects.</p>
