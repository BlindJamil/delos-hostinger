        ...
        <nav id="navbar" class="transition-all duration-500">
            <div class="max-w-[1400px] mx-auto px-6 lg:px-12">
                <div class="flex items-center justify-between h-18 lg:h-20">
                    ...
                    {{-- Mobile Hamburger --}}
                    <button id="mobile-menu-btn" class="lg:hidden relative z-50 flex flex-col gap-1.5 p-2"
            ...
            <div id="mobile-menu" class="lg:hidden fixed inset-0 top-0 bg-delos-dark/98 backdrop-blur-md z-40">
                <div class="flex flex-col justify-center items-center h-full px-8 py-20 gap-6 overflow-y-auto">
                    @foreach($links as $link)
                        <a href="{{ lroute($link['route']) }}"
                           class="font-serif text-delos-cream text-lg tracking-[0.15em] uppercase font-light hover:text-delos-gold transition-colors duration-300 text-center">
                            {{ $link['label'] }}
                        </a>
                    @endforeach
